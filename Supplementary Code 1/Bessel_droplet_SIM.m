...
% 1. flag_ScanR2, set flag_ScanR2=1 will change the R2 value from 0.2*R1 to
% R1, calculate the side-ring ratio and save the results as .tif files in the result folder.
% Based on the minimum side-ring ration, the optimized R2 can be found. 
%Then set the flag_ScanR2=0 to use the optimized R2 for simulation.

% 2. flag_saveTemResults=1? save the results during the simulation process.
% set flag_saveTemResults=0 when scan the R2.set flag_saveTemResults=1 when 
% simulate with the optimized R2 to generate the phase patterns.

% 3. flag_GaussianBeam=1; Genrally use the GaussianBeam profile.
% 4. flag_3D=1, save a 3D PSF.
% 5. flag_NormalizePSF=1;
% 6. flag_mask='A','B','DR', set the simulation for outer ring, inner ring
% or doule rings.
...
%% ###########    Cal Beam Size:
beamD=BeamDiameter; %mm, Beam size at the Bessel SLM
beadD_Pupil=beamD*(350/750)*10;%mm, Beam size at the objective back pupil
%% ############   Cal mObjective Pars: 
...
rOB=fOB*NA; % radius of objective back pupil, mm
...
for tt=1:1:TrialNum
outerD_B=(R2(tt)+RingThickness2/2)*2;
innerD_B=(R2(tt)-RingThickness2/2)*2;
...
%% Apply mask to the optical field at the back pupil plane
step_mask=3;% micron;
r_mask=0:step_Pupil:rOB*1000; 
...
PhasePhase_Pupil=ones(size(Amplitude_Pupil));%rad
...
%% Plot masked field at the pupil plane
...
filename=[resultPath, '\PupilFiled_masked_',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f')];
...
step_SLM=2.5;% um
pixel_SLM=15;% um
r_SLM=0:step_SLM:pixel_SLM*512*ceil(sqrt(2));%um
f1=200;%mm
...
filename=[resultPath, '\SLM_PhasePattern_',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f')];
...
%%
filename=[resultPath, '\SLM_PhaseMask_',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f')];
...
filename=[resultPath, '\PupilConfirm_',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f')];
...
filename=[resultPath, '\PSF_Profiles_R2_',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f'),'_Z=',num2str(z)];
...
% PSFWaitBar = waitbar(0,'0','Name','Calculation 3D PSF...');
...
%     waitbar(ii/length(z),PSFWaitBar,sprintf('%.2f%% finished',ii/length(z)));
end

%% Transfer to tiff image, Normalized to max value
[Max_3DPSF, I]=max(PSF_Cross_Stacks(:));
filename=[resultPath, '\PSF_3D',flag_mask,'_R2_',num2str(R2(tt),'%.3f'),'_R1_',num2str(R1,'%.3f'),'_dR2_',num2str(RingThickness2,'%.3f'),'_dR1_',num2str(RingThickness1,'%.3f')];
